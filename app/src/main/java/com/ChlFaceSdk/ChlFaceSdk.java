package com.ChlFaceSdk;

// 多通道人脸识别认证 SDK 接口类
public class ChlFaceSdk {
    ...
    // 备注：必须在SDK初始化前调用才有效
    //      本接口已过期，仅为兼容旧APP，新APP请改用新接口SetAuth
    public static native void SetUsbDogHandle(int nUsbDogType, int nUsbDogHandle);

    // 设置认证方式
    // 输入参数：
    //     nAuthType ---- 认证方式：0-红色或灰色USB加密狗或USB加密芯片，1-白色USB加密狗，2-板载I2C加密芯片，3-读卡器I2C加密芯片
    //     nUsbDogHandle ---- USB加密狗认证时指定加密狗设备句柄，需据此才能操作USB加密狗设备（可参考DEMO例程请求权限并获取设备句柄）
    //                        非USB加密狗认证时本参数无意义，可指定任何值
    // 备注：必须在SDK初始化前调用才有效
    //       原SetUsbDogHandle接口已过期，由本接口替代
